...
test("submitting the own-question form on the home page reaches /search with a classification", async ({
  ...
  await expect(page.getByText(/Danke — Bereich bestätigt/)).toBeVisible();
});
...
for (const [label, query] of [
  ["classification state", "Welche Lernmethode verbessert den Lernerfolg?"],
  ["restricted state", "Welche Chemotherapie hilft bei Krebs?"],
  ["no-question state", ""],
] as const) {
  test(`/search (${label}) has no critical accessibility violations @a11y`, async ({ page }) => {
    await page.goto(query ? `/search?q=${encodeURIComponent(query)}` : "/search");
